...
public class EntidadDAO implements IEntidadDAO {
    ...
    public boolean registrarEntidad(EntidadDTO dto) throws Exception {
        ...
        try {
            stmt = conn.prepareStatement("INSERT INTO `entidad_adscrita`(`entidad_nombre`, `entidad_NIT`, `entidad_dir`, `entidad_ciudad`, `entidad_fijo`, `entidad_movil`, `entidad_email`, `entidad_website`, `entidad_imagen`) VALUES (?,?,?,?,?,?,?,?,?);");
            ...
            stmt.setString(9, dto.getImagen());
    ...
    public ArrayList<EntidadDTO> consultarEntidad(String dato, String tipo) throws Exception {
        ...
        try {
            switch (tipo) {
                ...
                case "nit": {
                    stmt = conn.prepareStatement("SELECT * FROM entidad_adscrita WHERE entidad_NIT = ?");
                ...
                case "mun": {
                    stmt = conn.prepareStatement("SELECT * FROM entidad_adscrita WHERE entidad_ciudad = ?");
                    stmt.setString(1, dato);
                    ResultSet res = stmt.executeQuery();
                    while (res.next()) {
                        entidad = new EntidadDTO();
                ...
                case "tel": {
                    stmt = conn.prepareStatement("SELECT * FROM entidad_adscrita WHERE entidad_movil = ?");
                    stmt.setString(1, dato);
                    ResultSet res = stmt.executeQuery();
                    while (res.next()) {
                        entidad = new EntidadDTO();
